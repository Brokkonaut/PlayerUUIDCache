...
public class BinaryStorage {
    ...
    public BinaryStorage(File dataFolder, Logger logger) throws IOException {
        this.logger = logger;
        filePositions = new HashMap<>();
        file = new RandomAccessFile(getDatabaseFile(dataFolder), "rw");
    }

    public static File getDatabaseFile(File dataFolder) {
        return new File(dataFolder, "players.dat");
    }

    public ArrayList<CachedPlayerData> loadAllPlayers() throws IOException {
        if (loaded) {
            throw new IllegalStateException("loadAllPlayers can only be called once");
        }
        loaded = true;
        ArrayList<CachedPlayerData> players = new ArrayList<>();
        if (file.length() >= 4) {
            ...
            if (length > 0) {
                ...
                for (int i = 0; i < totalEntries; i++) {
                    ...
                    players.add(new CachedPlayerData(uuid, name, lastSeen, now));
    ...
    public void addOrUpdatePlayer(CachedPlayerData player) throws IOException {
        if (!loaded) {
    ...
    public void close() {
        try {
            ...
            logger.log(Level.SEVERE, "Error while trying to close the data file", e);
        }
    }
}
